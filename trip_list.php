...
// 1. Ana sayfadaki formdan gelen arama kriterlerini al
...
if (!empty($departure) && !empty($arrival) && !empty($date)) {
    try {
        // 2. Trips tablosunu Company tablosuyla birleştirerek (JOIN) arama yap
        // Bu sayede firma adını (company_name) da alabiliriz
        ...
        $stmt = $db->prepare($sql);
        // LIKE ile arama yapmak için % ekliyoruz (örn: "İstanbul" -> "%İstanbul%")
        $stmt->execute(['%' . $departure . '%', '%' . $arrival . '%', $date]);
        $trips = $stmt->fetchAll();

    } catch (PDOException $e) {
        $error_message = "Seferler aranırken bir hata oluştu: " . $e->getMessage();
    }
} else {
    $error_message = "Lütfen kalkış, varış ve tarih bilgilerini eksiksiz girin.";
}
?>
<!DOCTYPE html>
<html lang="tr">
...
<body>
    ...
    <main class="content-area">
        <div class="search-widget" style="margin-bottom: 20px;">
             <form action="sefer_listesi.php" method="GET" class="search-form">
                <div class="form-group">
                    ...
                    <input type="text" id="kalkis" name="departure_location" value="<?php echo htmlspecialchars($departure); ?>" required>
                </div>
                <div class="form-group">
                    <label for="varis">Nereye?</label>
                    <input type="text" id="varis" name="arrival_location" value="<?php echo htmlspecialchars($arrival); ?>" required>
                </div>
                <div class="form-group">
                    <label for="tarih">Tarih</label>
                    <input type="date" id="tarih" name="date" value="<?php echo htmlspecialchars($date); ?>" required>
                </div>
                <button type="submit" class="search-button">Sefer Ara</button>
            </form>
        </div>

        <h2>Arama Sonuçları: <?php echo htmlspecialchars($departure); ?> -> <?php echo htmlspecialchars($arrival); ?></h2>
        ...
        <?php if (empty($trips) && !$error_message): ?>
            <p style="text-align: center; font-size: 18px;">Bu kriterlere uygun sefer bulunamadı.</p>
        <?php endif; ?>
